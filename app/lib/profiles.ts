...
export async function getFilteredProfiles(params: {
  ...
  return PROFILES_DATA.filter((profile) => {
    ...
    if (params.ageRange) {
      const [minAge, maxAge] = params.ageRange.split('-').map((val) => parseInt(val.trim(), 10));
    ...
    if (params.religion && profile.religion) {
      const selectedReligions = params.religion.split(',').map((r) => r.trim().toLowerCase());
    ...
    // 5. Education check
    if (params.education && !profile.education.toLowerCase().includes(params.education.toLowerCase())) {
      return false;
    }

    // 6. Interests / Tags check (comma separated: "Fine Arts")
    if (params.interests) {
      const requestedInterests = params.interests.split(',').map((i) => i.trim().toLowerCase());
      const profileTags = profile.tags.map((t) => t.toLowerCase());
      const hasMatchingInterest = requestedInterests.some((interest) => profileTags.includes(interest));
